...
| `task-default-isolation-detection.md` | SE-0466's `-default-isolation` value is never read from the real project — always resolves `nonisolated` ([issue #30](https://github.com/btctcn/swift-isolation-map/issues/30)) | **Open** — hypothesis confirmed, SwiftPM path spiked and confirmed end-to-end; Xcode path and the rule-set/compiler-args construction ordering still need deciding before implementation |
