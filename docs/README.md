...
| `task-closure-isolation-attribution.md` | Standalone, external-reader-friendly writeup of the closure-attribution gap ([issue #33](https://github.com/btctcn/swift-isolation-map/issues/33)): root cause, exactly which closure forms are confirmed to trigger it, why it's structurally scoped to source this tool parses itself, and the proposed (not yet built) fix design | **Open** — investigated and root-caused, fix design proposed and awaiting explicit approval before any implementation, per this project's workflow |
